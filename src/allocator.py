...
from .build_objects import (
    rat_list, forecast_dict,
    board_inventory_list, inv_med_list, inv_rec_stamped_list, inv_any_unstamped_list, 
    ForecastKey,
    sim
)
...
final_inv = pd.DataFrame([
    ...
    for inv in board_inventory_list + inv_rec_stamped_list + inv_any_unstamped_list 
])
...
def short_reason_tuple(inv_list, part, prov, channel, remaining):
    ...
    if total_aged >= remaining:
        ...
        reason = "no inventory (fresh or aged)"
...
inventory_start_df = inv_snapshot(board_inventory_list + inv_med_list + inv_rec_stamped_list + inv_any_unstamped_list)
        ...
        for prov in prov_list:
            for channel in channel_list:
                ...
                if fcval: #key match
                    ...
                    remaining = fifo_inventory_list(board_inventory_list, part, prov, channel, pod, demand, SHIPMENTLOG)
                    remaining = fifo_inventory_list(inv_med_list, part, prov, channel, pod, remaining, SHIPMENTLOG)
                    ...
                    if remaining == 0:
                        ...
                        all_inv = board_inventory_list + inv_med_list + inv_rec_stamped_list + inv_any_unstamped_list
...
inventory_end_df = inv_snapshot(board_inventory_list + inv_med_list + inv_rec_stamped_list + inv_any_unstamped_list)
